...
class UserController extends AppController
{
    ...
    public function actionCreate()
    {
        ...
        if($file = UploadedFile::getInstance($user,'image')){
            $user->image = $user->upload(strtolower(Yii::$app->controller->id).'ss',$file);
        }
        if (!$user->save()) {
            return $this->render('create', ['user' => $user]);
        }
        ...
        if (!$info) {
            Yii::$app->session->setFlash('error', Yii::t('yii', 'There was some error while saving user role.'));
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if($file = UploadedFile::getInstance($user,'image')){
                $user->image = $user->upload(strtolower(Yii::$app->controller->id).'ss',$file);
                if(file_exists($user->getOldAttribute('image')))
                    unlink($user->getOldAttribute('image'));
            }else{
    ...
    public function actionDelete($id)
    {
        ...
        if (!$this->findModel($id)->delete()) {
            throw new ServerErrorHttpException(Yii::t('yii', 'We could not delete this user.'));
        }
        ...
        if (!$info) {
            Yii::$app->session->setFlash('error', Yii::t('yii', 'There was some error while deleting user role.'));
            return $this->redirect(['index']);
        }

        Yii::$app->session->setFlash('success', Yii::t('yii', 'You have successfuly deleted user and his role.'));
