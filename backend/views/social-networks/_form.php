<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;

/* @var $this yii\web\View */
/* @var $model backend\models\SocialNetworks */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="social-networks-form">

    <?php $form = ActiveForm::begin(); ?>

    <?= $form->field($model, 'class')->dropDownList([ 'facebook' => 'Facebook', 'instagram' => 'Instagram', 'google-plus' => 'Google-plus', 'telegram' => 'Telegram', ], ['prompt' => 'My Network']) ?>

     <?= $form->field($model, 'url')->textarea(['placeholder' => Yii::t('yii','@telegram_instagram')]) ?>

    <?= $form->field($model, 'status')->radioList([ '1' => Yii::t('yii','Active'), '0' => Yii::t('yii','Inactive'), ]) ?>

    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? Yii::t('yii', 'Save') : Yii::t('yii', 'Update'), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>
